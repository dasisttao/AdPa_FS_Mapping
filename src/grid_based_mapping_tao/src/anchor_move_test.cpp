...
namespace ACS
{
    ...
    double grid_border_x1=(GRID_SIZE_X*100.0/400)*GRID_SPACING;
    double grid_border_x2=(GRID_SIZE_X*300.0/400)*GRID_SPACING;
    double grid_border_y1=(GRID_SIZE_Y*100.0/400)*GRID_SPACING;
    double grid_border_y2=(GRID_SIZE_Y*300.0/400)*GRID_SPACING;
...
void update_grid_when_anchor_moved(float (*grid)[GRID_SIZE_X][GRID_SIZE_Y],float new_are_fullfill)
{
    ...
    ROS_INFO("GRID_TRANS_Y= %d",grid_trans_y);
    ROS_INFO("GRID_TRANS_X= %d",grid_trans_x);
     /* 
...
*/
    ...
    {   
        for(auto i=0;i<400;i++)
            for(auto j=400;j>grid_trans_y;j--)
            { 
