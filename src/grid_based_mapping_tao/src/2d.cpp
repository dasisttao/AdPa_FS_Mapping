...
uint8_t sense_grid[GRID_SIZE_X][GRID_SIZE_Y];
...
void freespace_debug2(uint8_t (*current_grid)[GRID_SIZE_X][GRID_SIZE_Y],uint8_t (*history_grid)[GRID_SIZE_X][GRID_SIZE_Y]);
...
{
  double DMAX=15; //max distannce of reachement
  double DMIN=2;
  ...
  //std::vector<double> laser_x {+3.3}; //In VCS Vehicle Coordinate System
  //std::vector<double> laser_y {-0.8};
  //std::vector<double> laser_yaw {-125*M_PI/180};
...
int main(int argc, char **argv)
{
  ...
  uint8_t (*sense_grid_pointer)[GRID_SIZE_X][GRID_SIZE_Y]=&sense_grid;
  ...
  while(ros::ok())
  {
    ...
    //adj(sense_grid_pointer);
    adj(testt_grid_pointer);
    
    freespace_debug2(current_grid_pointer,history_grid_pointer);

    #ifdef PUBLISH_DATA_
    data_to_pub.car_points=car_cluster;
    #endif
      
    adj(history_grid_pointer);
    ...
    for(int i=0;i<GRID_SIZE_X;i++)
      ...
      visualize_girdcells(pcl_cluster_temp,"map",float(GRID_SPACING),float(GRID_SPACING),occupied_area_pub,VISUAL_OFFSET_X,VISUAL_OFFSET_Y);
    ...
    */
    ...
    for(int num=0;num<(radiation_num/radiation_num_divder);num++)
    {
      ...
      */
      ...
      for(int32_t j=0;j<(deltafastdirection-1);j++)//Check if i have the obstabcle
      {    
        err=err-deltaslowdirection;
        if(err<0)
        {
        ...
        if(test_grid[uint16_t(x)][uint16_t(y)]==90)
        {
          if_obstacle=true;
          break;
        }
      ...
      {   
          for(int32_t j=0;j<(deltafastdirection-1);j++)
          {
            ...
            if(test_grid[uint16_t(x)][uint16_t(y)]>binary_threshold)
              {
                double d=sqrt( pow((x-xstart)*GRID_SPACING,2.0)+ pow((y-ystart)*GRID_SPACING,2.0) );
    ...
    */
    ...
    for(int num=0;num<(radiation_num/radiation_num_divder);num++)
    {
      ...
      */
      ...
      if(if_obstacle==true)
      {   
          for(int32_t j=0;j<(deltafastdirection);j++)
          {
