...
void create_freespace(uint8_t(*ray_casting)[GRID_SIZE_X][GRID_SIZE_Y],uint8_t(*pcl_grid)[GRID_SIZE_X][GRID_SIZE_Y],uint8_t(*current_grid)[GRID_SIZE_X][GRID_SIZE_Y]);
...
ros::Publisher unknown_pub = nh.advertise<nav_msgs::GridCells>("unknown_area",1000);
  ...
  while(ros::ok())
  {
    ...
    if(if_get_new_data==true)
    {
      ...
      for(int i=0;i<car_grid.size();i++)
      {
       ...
       pcl_grid[uint16_t(car_grid[i].x)][uint16_t(car_grid[i].y)]=33;//car
      }
      ...
      create_freespace(ray_casting_pointer,pcl_grid_pointer,current_grid_pointer);
      ...
      std::vector<geometry_msgs::Point> unknown_grids;
        ...
        for(int j=0;j<GRID_SIZE_Y;j++)
          {
            ...
            else if((history_grid[i][j]==50)&&(current_grid[i][j]!=10))//grid[i][j]=g_data.array.data[i*GRID_SIZE_Y+j];
            {
               
              cluster_point.x=ACS::anchor_x+i*GRID_SPACING;
              cluster_point.y=ACS::anchor_y+j*GRID_SPACING;
              cluster_point.z=0.0;
              unknown_grids.push_back(cluster_point);
            }
            else if(history_grid[i][j]<50)//grid[i][j]=g_data.array.data[i*GRID_SIZE_Y+j];
            {
    ...
    visualize_girdcells(unknown_grids,"map",float(GRID_SPACING),float(GRID_SPACING),unknown_pub,VISUAL_OFFSET_X,VISUAL_OFFSET_Y);
 ...
 void callback(const  geometry_msgs::PoseStamped::ConstPtr& pose_msg,const pcl::PointCloud<pcl::PointXYZL>::ConstPtr& pcl_msg,uint8_t(*pcl_grid)[GRID_SIZE_X][GRID_SIZE_Y])
{
    ...
    if(abs(diff)<allowed_diff_ts_ns)//Data is valid
    {
        ...
        */
        ...
        BOOST_FOREACH (const pcl::PointXYZL& pt, pcl_msg->points)
        {
          //if((pt.label!=2)&&(pt.label!=3)&&(pt.label!=1)&&(pt.label!=0)) //We acess only layer 2-3
          //{continue;} 
