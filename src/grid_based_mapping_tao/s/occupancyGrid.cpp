...
{
  double DMAX=12; //max distannce of reachement
  double DMIN=1;
...
void ray_casting_points(uint8_t(*pcl_grid)[GRID_SIZE_X][GRID_SIZE_Y],uint8_t(*current_grid)[GRID_SIZE_X][GRID_SIZE_Y]);
void ray_casting_points_show_ray(uint8_t(*pcl_grid)[GRID_SIZE_X][GRID_SIZE_Y],uint8_t(*current_grid)[GRID_SIZE_X][GRID_SIZE_Y],uint8_t(*ray_casting)[GRID_SIZE_X][GRID_SIZE_Y]);
void binary_bayes(uint8_t (*current_grid)[GRID_SIZE_X][GRID_SIZE_Y],uint8_t (*history_grid)[GRID_SIZE_X][GRID_SIZE_Y]);
//++++++++++++++++++++++++++++++++++++++++++++++++++MAIN++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher unknow_pub = nh.advertise<nav_msgs::GridCells>("unknow_area",1000);
  ros::Publisher free_space_pub = nh.advertise<nav_msgs::GridCells>("free_space",1000);
  
  #ifdef SHOW_RAY
  ros::Publisher ray_see_obj_pub = nh.advertise<nav_msgs::GridCells>("ray_see_obj",1000);
  ros::Publisher ray_see_nothing_pub = nh.advertise<nav_msgs::GridCells>("ray_see_nothing",1000);
  #endif
  ...
  while(ros::ok())
  {
    ...
    if(if_get_new_data==true)
    {
      ...
      //map
      #ifdef SHOW_RAY
      ray_casting_points_show_ray(pcl_grid_pointer,current_grid_pointer,ray_casting_pointer);
      #else
      ray_casting_points(pcl_grid_pointer,current_grid_pointer);
      #endif
      ...
      std::vector<geometry_msgs::Point> occupied_grids;
      std::vector<geometry_msgs::Point> free_grids;
      std::vector<geometry_msgs::Point> unknow_grids;
      #ifdef SHOW_RAY
      std::vector<geometry_msgs::Point> ray_see_obj;
      std::vector<geometry_msgs::Point> ray_see_nothing;
      #endif

      for(int i=0;i<GRID_SIZE_X;i++)
        for(int j=0;j<GRID_SIZE_Y;j++)
          {
            if(history_grid[i][j]>50)//grid[i][j]=g_data.array.data[i*GRID_SIZE_Y+j];
            {
               
              cluster_point.x=ACS::anchor_x+i*GRID_SPACING;
              cluster_point.y=ACS::anchor_y+j*GRID_SPACING;
              cluster_point.z=0.0;
              occupied_grids.push_back(cluster_point);
            }
            if(history_grid[i][j]==50)//grid[i][j]=g_data.array.data[i*GRID_SIZE_Y+j];
            {
              ...
              cluster_point.y=ACS::anchor_y+j*GRID_SPACING;
              cluster_point.z=0.0;
              unknow_grids.push_back(cluster_point);
            }
            if(history_grid[i][j]<50)//grid[i][j]=g_data.array.data[i*GRID_SIZE_Y+j];
            {
              ...
              cluster_point.y=ACS::anchor_y+j*GRID_SPACING;
              cluster_point.z=0.0;
              free_grids.push_back(cluster_point);
            }
            ...
            if(ray_casting[i][j]==90)
            {
              ...
              cluster_point.y=ACS::anchor_y+j*GRID_SPACING;
              cluster_point.z=0.0;
              ray_see_obj.push_back(cluster_point);
            }
            if(ray_casting[i][j]==0)
            {
              cluster_point.x=ACS::anchor_x+i*GRID_SPACING;
              cluster_point.y=ACS::anchor_y+j*GRID_SPACING;
              cluster_point.z=0.0;
              ray_see_nothing.push_back(cluster_point);
            }
            #endif
      ...
      visualize_girdcells(occupied_grids,"map",float(GRID_SPACING),float(GRID_SPACING),occupied_area_pub,VISUAL_OFFSET_X,VISUAL_OFFSET_Y);
      visualize_girdcells(unknow_grids,"map",float(GRID_SPACING),float(GRID_SPACING),unknow_pub,VISUAL_OFFSET_X,VISUAL_OFFSET_Y);
      visualize_girdcells(free_grids,"map",float(GRID_SPACING),float(GRID_SPACING),free_space_pub,VISUAL_OFFSET_X,VISUAL_OFFSET_Y);
      #ifdef SHOW_RAY
      visualize_girdcells(ray_see_obj,"map",float(GRID_SPACING),float(GRID_SPACING),ray_see_obj_pub,VISUAL_OFFSET_X,VISUAL_OFFSET_Y);
      visualize_girdcells(ray_see_nothing,"map",float(GRID_SPACING),float(GRID_SPACING),ray_see_nothing_pub,VISUAL_OFFSET_X,VISUAL_OFFSET_Y);
      #endif
    ...
    for(int num=0;num<(radiation_num/radiation_num_divder);num++)
    {
      ...
      if(if_obstacle==true)
      {
        ...
        {    
          ...
          if((*current_grid)[uint16_t(x)][uint16_t(y)]==90)
          {
            break;       
          }
    ...
    for(int num=0;num<(radiation_num/radiation_num_divder);num++)
    {
      ...
      if(if_obstacle==true)
      {
        ...
        {    
          ...
          else
          {
           ...
           (*ray_casting)[uint16_t(x)][uint16_t(y)]=90;
          if((*current_grid)[uint16_t(x)][uint16_t(y)]==90)
          {
            break;       
          }
      ...
      if(if_obstacle==false)
      {
        ...
        {    
          ...
          (*ray_casting)[uint16_t(x)][uint16_t(y)]=0;
          double d=sqrt( pow((x-xstart)*GRID_SPACING,2.0)+ pow((y-ystart)*GRID_SPACING,2.0) );
              if (d<DMIN)
              {
                (*current_grid)[uint16_t(x)][uint16_t(y)]=20;
  ...
  for(int i=0;i<GRID_SIZE_X;i++)
    for(int k=0;k<GRID_SIZE_Y;k++)
        { 
          ...
          {
            double p_cur=double((*current_grid)[i][k]);
            double p_hist=double((*history_grid)[i][k]);
